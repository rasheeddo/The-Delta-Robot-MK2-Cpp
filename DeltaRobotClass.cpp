...
DeltaRobotClass::DeltaRobotClass() 
...
float DeltaRobotClass::MAP(uint32_t angle, long in_min, long in_max, long out_min, long out_max)
{
	return (((float)angle - in_min) * (out_max - out_min) / (in_max-in_min) + out_min);
};

void DeltaRobotClass::printMatrix(double matrix[][4], int size)
{
...
void DeltaRobotClass::printMatrix3(double matrix[][3], int size)
{
	//Print matrix: (square) // cut into 2 function ? add print line?
	for(int l=0; l<size; l++)
	{
...
void DeltaRobotClass::multiplyMatrix(double m1[][4], double m2[][4], double m12[][4], int size)
{
	for (int l=0; l<size; l++)
	{
		for(int c=0; c<size; c++)
...
void DeltaRobotClass::multiplyMatrix3(double m1[][3], double m2[][3], double m12[][3], int size)
{
	for (int l=0; l<size; l++)
	{
		for(int c=0; c<size; c++)
...
void DeltaRobotClass::FWD(float robotAngle[ ], float positionXYZ[ ])
{
	//Convertion
	...
	//Matrix creation:
	double A1v[3] = {0, -wb - L*cos(deg1) + up, -L*sin(deg1)};
	double A2v[3] = {(ROOT3/2)*(wb + L*cos(deg2)) - sp/2, 0.5*(wb + L*cos(deg2)) - wp, -L*sin(deg2)};
	double A3v[3] = {(-ROOT3/2)*(wb + L*cos(deg3)) + sp/2, 0.5*(wb + L*cos(deg3)) - wp, -L*sin(deg3)};
	...
	switch(method)
	{
		case 1:
			...
			a = 2*(x3 - x1);
			b = 2*(y3 - y1);
			c = pow(r1,2) - pow(r3,2.0) - pow(x1,2.0) - pow(y1,2.0) + pow(x3,2.0) + pow(y3,2.0);
			d = 2*(x3 - x2);
			e = 2*(y3 - y2);
			f = pow(r2,2) - pow(r3,2.0) - pow(x2,2.0) - pow(y2,2.0) + pow(x3,2.0) + pow(y3,2.0);
			numX = (c*e) - (b*f);
			denX = (a*e) - (b*d);
			x[0] = numX/denX;
			if(x[0] < 0.000001){
		...
		case 2:
			...
			b1 = pow(r1,2) - pow(r3,2) - pow(x1,2) - pow(y1,2) - pow(z1,2) + pow(x3,2) + pow(y3,2) + pow(z3,2);
			b2 = pow(r2,2) - pow(r3,2) - pow(x2,2) - pow(y2,2) - pow(z2,2) + pow(x3,2) + pow(y3,2) + pow(z3,2);
			...
			c = a5*(a5 - 2*x1) + a7*(a7 - 2*z1) + pow(x1,2) + pow(y1,2) + pow(z1,2) - pow(r1,2);
		...
		case 3:
			...
			b1 = pow(r2,2) - pow(r1,2) - pow(x2,2) - pow(y2,2) - pow(z2,2) + pow(x1,2) + pow(y1,2) + pow(z1,2);
			b2 = pow(r3,2) - pow(r1,2) - pow(x3,2) - pow(y3,2) - pow(z3,2) + pow(x1,2) + pow(y1,2) + pow(z1,2);
		...
		case 4:
			...
			b1 = pow(r1,2) - pow(r2,2) - pow(x1,2) - pow(y1,2) - pow(z1,2) + pow(x2,2) + pow(y2,2) + pow(z2,2);
			b2 = pow(r3,2) - pow(r2,2) - pow(x3,2) - pow(y3,2) - pow(z3,2) + pow(x2,2) + pow(y2,2) + pow(z2,2);
...
void DeltaRobotClass::INV(float wantedXYZ[ ], float outputRobotAngle[ ]) 
{
...
uint16_t DeltaRobotClass::GetModelNumber(int ID)
{
	uint16_t modelNumber=0;
	dxl_comm_result=packetHandler->read2ByteTxRx(portHandler, ID, ADDR_PRO_MODEL, &modelNumber);
...
void DeltaRobotClass::SetOperatingMode(int ID, uint8_t mode)
{
	TorqueOFF(ID);

	if( (mode==0) || (mode==1) || (mode==3) || (mode==4) || (mode==5) || (mode==16))
	{
		dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, ID, ADDR_PRO_OPERATING_MODE, mode, &dxl_error);
		if (dxl_comm_result != COMM_SUCCESS)
		{
			printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
	    }
...
void DeltaRobotClass::PrintOperatingMode(int ID)
{
	uint8_t mode=3;
	dxl_comm_result = packetHandler->read1ByteTxRx(portHandler, ID, ADDR_PRO_OPERATING_MODE, &mode, &dxl_error);
	switch(mode)
	{
	case 0:
...
void DeltaRobotClass::SetDrivingMode(int ID, uint8_t type)
{
	dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, ID, ADDR_PRO_DRIVE_MODE, type);
	PrintDrivingMode(ID);
}

uint8_t DeltaRobotClass::PrintDrivingMode(int ID)
{
	uint8_t type;
	dxl_comm_result = packetHandler->read1ByteTxRx(portHandler, ID, ADDR_PRO_DRIVE_MODE, &type);
	printf("Motor %d is on driving mode: %d\n", ID, type);
	return(type);
}

uint8_t DeltaRobotClass::IsMoving(int ID)
{
	uint8_t moving;
	dxl_comm_result = packetHandler->read1ByteTxRx(portHandler, ID, ADDR_PRO_MOVING, &moving);
	return (moving);
}

void DeltaRobotClass::MovingStatus(int ID)
{
	uint8_t movingstatus=0;
	dxl_comm_result = packetHandler->read1ByteTxRx(portHandler, ID, ADDR_PRO_MOVING_STATUS, &movingstatus);
	if(movingstatus > 48)
	{
...
void DeltaRobotClass::SetPID(int ID, uint16_t P_gain, uint16_t I_gain, uint16_t D_gain)
{
	dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, ID, ADDR_PRO_POSITION_P_GAIN, P_gain, &dxl_error);
	dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, ID, ADDR_PRO_POSITION_I_GAIN, I_gain, &dxl_error);
	dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, ID, ADDR_PRO_POSITION_D_GAIN, D_gain, &dxl_error);
	// check if PID are set:
	PrintPID(ID);
}
void DeltaRobotClass::PrintPID(int ID)
{
	uint16_t P, I, D;
	dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, ID, ADDR_PRO_POSITION_P_GAIN, &P, &dxl_error);
	dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, ID, ADDR_PRO_POSITION_I_GAIN, &I, &dxl_error);
	dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, ID, ADDR_PRO_POSITION_D_GAIN, &D, &dxl_error);
	printf("Motor %d: PID values : P: %d / I: %d / D: %d\n", ID, P, I, D);
}

void DeltaRobotClass::SetFFGain(int ID, uint16_t FF1Gain, uint16_t FF2Gain)
{
	dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, ID, ADDR_PRO_FEEDFORWARD_1st_GAIN, FF1Gain);
	dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, ID, ADDR_PRO_FEEDFORWARD_2nd_GAIN, FF2Gain);
	// check if FFgain are set:
	PrintFFGain(ID);
}
void DeltaRobotClass::PrintFFGain(int ID)
{
	uint16_t FF1Gain, FF2Gain;
	dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, ID, ADDR_PRO_FEEDFORWARD_1st_GAIN, &FF1Gain);
	dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, ID, ADDR_PRO_FEEDFORWARD_2nd_GAIN, &FF2Gain);
	printf("Motor %d, Feed forward Gain: FF1: %d / FF2: %d\n", ID, FF1Gain, FF2Gain);
}
		
void DeltaRobotClass::SetProfile(int ID, uint32_t Velocity, uint32_t Acceleration)
{
	//Set the limits:
	dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, ID, ADDR_PRO_VELOCITY_LIMIT, VELOCITY_LIMIT);
	dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, ID, ADDR_PRO_ACCELERATION_LIMIT, ACCELERATION_LIMIT);
	if(Velocity<=VELOCITY_LIMIT)
	{
		dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, ID, ADDR_PRO_PROFILE_VELOCITY, Velocity);
	}
	else 
	{
	...
	{
		dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, ID, ADDR_PRO_PROFILE_ACCELERATION, Acceleration);
	}
	else
	{
...
void DeltaRobotClass::PrintProfile(int ID)
{
	uint32_t acceleration=0;
	uint32_t velocity=0;
	dxl_comm_result = packetHandler->read4ByteTxRx(portHandler, ID, ADDR_PRO_PROFILE_VELOCITY, &velocity);
	dxl_comm_result = packetHandler->read4ByteTxRx(portHandler, ID, ADDR_PRO_PROFILE_ACCELERATION, &acceleration);
	printf("Motor %d:	Acceleration Profile: %d and Velocity Profile: %d\n", ID, acceleration, velocity);
} 

void DeltaRobotClass::SetTimeProfile(int ID, uint32_t Ta, uint32_t Tf)
{
	dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, ID, ADDR_PRO_PROFILE_ACCELERATION_TIME, Ta);
	dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, ID, ADDR_PRO_PROFILE_TIME_SPAN, Tf);
	PrintTimeProfile(ID);
}


void DeltaRobotClass::PrintTimeProfile(int ID)
{
	uint32_t Ti, Tf;
	dxl_comm_result = packetHandler->read4ByteTxRx(portHandler, ID, ADDR_PRO_PROFILE_ACCELERATION_TIME, &Ti);
	dxl_comm_result = packetHandler->read4ByteTxRx(portHandler, ID, ADDR_PRO_PROFILE_TIME_SPAN, &Tf);
	printf("Motor %d, acceleration time: %d / timespan: %d\n", ID, Ti, Tf);
}

void DeltaRobotClass::SetCurrentLimit(int ID, uint32_t current_limit)
{
	dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, ID, ADDR_PRO_CURRENT_LIMIT, current_limit);
}
void DeltaRobotClass::PrintCurrentLimit(int ID)
{
	float SetUnit = 1;
	...
	dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, ID, ADDR_PRO_CURRENT_LIMIT, &current_limit);
	current_limit_amp=(float)current_limit*(SetUnit/1000.0);
	printf("Motor %d, Current limit: %d (input motor), %f (amp)\n", ID, current_limit, current_limit_amp);
}

void DeltaRobotClass::SetGoalCurrent(int ID, uint16_t GoalCurrent)
{
	if((GoalCurrent>=-CURRENT_LIMIT) || (GoalCurrent<=CURRENT_LIMIT))
	{
		dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, ID, ADDR_PRO_GOAL_CURRENT, GoalCurrent);
	}
	else
	{
...
void DeltaRobotClass::PrintGoalCurrent(int ID)
{
uint16_t goalCurrent=0;
dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, ID, ADDR_PRO_GOAL_CURRENT, &goalCurrent);
printf("Motor %d: Goal Current: %d mA\n", ID, goalCurrent);
}

void DeltaRobotClass::TorqueON(int ID)
{
...
void DeltaRobotClass::TorqueOFF(int ID)
{
...
*/
void DeltaRobotClass::DriveAngle(int ID, float position)
{
	position=(int)MAP(position, 0, 360, MIN_MOTOR_ANGLE, MAX_MOTOR_ANGLE);
	dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, ID, ADDR_PRO_GOAL_POSITION, position, &dxl_error);
}

void DeltaRobotClass::SyncDriveAngle(float Deg123[ ])
{
	...
	dxl_goal_position1 = (int)MAP(Deg123[0], 0, 360, MIN_MOTOR_ANGLE, MAX_MOTOR_ANGLE);
	dxl_goal_position2 = (int)MAP(Deg123[1], 0, 360, MIN_MOTOR_ANGLE, MAX_MOTOR_ANGLE);
	dxl_goal_position3 = (int)MAP(Deg123[2], 0, 360, MIN_MOTOR_ANGLE, MAX_MOTOR_ANGLE);
...
void DeltaRobotClass::SyncReadAngle(float allAngle[ ])
{
...
void DeltaRobotClass::RobotInit(int mode)
{	
...
void DeltaRobotClass::RobotTorqueON()
{
	TorqueON(DXL1_ID);
	TorqueON(DXL2_ID);
	TorqueON(DXL3_ID);
...
void DeltaRobotClass::RobotTorqueOFF()
{
...
void DeltaRobotClass::GripperTorqueON()
{
	TorqueON(DXL4_ID);
	printf("GripperTorqueON: TORQUE ON\n");
}

void DeltaRobotClass::GripperTorqueOFF()
{
	TorqueOFF(DXL4_ID);
	printf("GripperTorqueOFF: TORQUE OFF\n");
}

void DeltaRobotClass::GripperOpen()
{
	dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, DXL4_ID, ADDR_PRO_GOAL_POSITION, GRIPPER_OPEN, &dxl_error);
	printf("GripperOpen: Open\n");
}

void DeltaRobotClass::GripperClose()
{
	dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, DXL4_ID, ADDR_PRO_GOAL_POSITION, GRIPPER_CLOSE, &dxl_error);
	printf("GripperClose: Close\n");
}

void DeltaRobotClass::GripperCheck()
{
	GripperOpen();
	usleep(1000000);
	GripperClose();
...
void DeltaRobotClass::IsAllStop()
{
	int MovingFlag = 1;
	int delayMicrosec;
	...
	{	
		...
		//printf("IsMoving2: %d \n", IsMoving(DXL2_ID));
		//printf("IsMoving3: %d \n", IsMoving(DXL3_ID));
		if ( (IsMoving(DXL1_ID) == 0) && (IsMoving(DXL2_ID) == 0) && (IsMoving(DXL3_ID) == 0))
		{	
			MovingFlag = 0;
...
void DeltaRobotClass::GotoPoint(float X, float Y, float Z)
{
	int WARN = 0;
	float wantedXYZ[3] = {X, Y, Z};
	float GoalAngle[3];
...
void DeltaRobotClass::GetXYZ(float currentXYZ[ ])
{
	float servoAngle[3];
	float robotAngle[3];
...
void DeltaRobotClass::GetRobotAngle(float robotAngle[ ])
{
	float servoAngle[3];
	SyncReadAngle(servoAngle);
...
void DeltaRobotClass::GoHome()
{
	float ServoHomePos[3] = {180.0, 180.0, 180.0};
...
void DeltaRobotClass::KinematicsCheck()
{
	float RobotAngle[3];
